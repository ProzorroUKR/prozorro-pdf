...
export const pdfDictionaryMap = new Map<string, Map<string, string>>()
  ...
  .set(
    ...
    tenderRejectionProtocolDictionaries
  )
  .set(
    PdfTemplateTypes.ANNUAL_PROCUREMENT_PLAN,
    annualProcurementPlanDictionaries
  )
  .set(
    PdfTemplateTypes.DETERMINING_WINNER_OF_PROCUREMENT_TEMPLATE,
    determiningWinnerOfProcurementDictionaries
  )
  .set(
    PdfTemplateTypes.PURCHASE_CANCELLATION_PROTOCOL_TEMPLATE,
    purchaseCancellationProtocolDictionaries
  )
  .set(
    PdfTemplateTypes.PROTOCOL_CONSIDERATION_TENDER_OFFERS_TEMPLATE,
    protocolConsiderationTenderOffersDictionaries
  )
  .set(
    PdfTemplateTypes.PROTOCOL_ON_EXTENSION_OF_REVIEW_PERIOD_TEMPLATE,
    protocolOnExtensionOfReviewPeriodDictionaries
  )
