...
export class DataTypeValidator implements DataTypeValidatorInterface {
  ...
  private readonly _factory = new TypeCheckFactory(
    typeCheckStrategyMap,
    this._typeChecker,
    this._emptyChecker
  );
  ...
  validate(
    ...
    errorMessage?: string,
    errorCode: ERROR_CODES = ERROR_CODES.INVALID_PARAMS
  ): void {
    if (!this._typeChecker.isArray(typesList)) {
      typesList = [typesList as string];
    }

    const valid = (typesList as string[]).some((type: string) =>
      this._checkType(data, type)
    );
    ...
    if (!valid) {
      throw new ErrorExceptionCore({
        code: errorCode,
        message:
          errorMessage || ERROR_MESSAGES.INVALID_PARAMS.incorrectInputFormat,
      });
    }
  }
