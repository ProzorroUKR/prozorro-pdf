export const PROTOCOL_CONSIDERATION_TENDER_OFFERS: Record<string, string> = {
  ...
  name_of_the_nomenclature_item_of_the_procurement:
    "Найменування учасника (для юридичної особи) або прізвище, ім’я, по батькові (для фізичної особи)",
  result_of_consideration_of_tender_offer:
    "Результат розгляду тендерної пропозиції  (відхилення тендерної пропозиції/допущення до аукціону)",
  ...
  has_been_resolved_text:
    "Прийняти та затвердити рішення щодо розгляду тендерних пропозицій на відповідність вимогам тендерної документації до проведення оцінки тендерних пропозицій та опублікувати протокол розгляду тендерних пропозицій на веб-порталі Уповноваженого органу.",
